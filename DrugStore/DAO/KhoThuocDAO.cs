...
namespace DAO
{
    public class KhoThuocDAO
    {
        ...
        {
            String query = string.Format("SELECT Thuoc.SODK, Thuoc.TENTHUOC, MAPHIEU, HSD, SOLUONG FROM KhoThuoc JOIN Thuoc ON Thuoc.SODK = KhoThuoc.SODK WHERE DATEDIFF(DAY, GETDATE(), HSD) >= 0 AND (SOLUONG BETWEEN 1 AND 50)");
            return DataProvider.Instance.ExecuteQuery(query);
        }
    }
}
