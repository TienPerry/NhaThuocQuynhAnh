...
namespace DAO
{
    public class ThuocDAO
    {
        ...
        {
            String query = "select t.SODK, t.TENTHUOC, kt.MAPHIEU, HSD, kt.SOLUONG from Thuoc t join KhoThuoc kt ON t.SODK = kt.SODK WHERE DATEDIFF(DAY,GETDATE(),kt.HSD) BETWEEN @hsd1 AND @hsd2 AND kt.SOLUONG > 0 AND DATEDIFF(DAY,GETDATE(),kt.HSD) > 0";
            return DataProvider.Instance.ExecuteQuery(query, new object[] { hsd1, hsd2 });
        }
        public DataTable thongKeThuocHHTrongKho()
        {
            return DataProvider.Instance.ExecuteQuery("SELECT t.SODK, t.TENTHUOC, kt.MAPHIEU, HSD, kt.SOLUONG from Thuoc t join KhoThuoc kt ON t.SODK = kt.SODK WHERE kt.SOLUONG > 0 AND DATEDIFF(DAY,GETDATE(),kt.HSD) <= 0");
        }
